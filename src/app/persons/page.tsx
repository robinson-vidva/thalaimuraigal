...
export default function PersonsPage() {
  ...
  const generations = [
    ...new Set(persons.map((p) => p.generation).filter((g) => g !== null)),
  ].sort((a, b) => a - b);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          href="/persons/new"
          className="bg-amber-700 text-white px-4 py-2 rounded-md hover:bg-amber-800 text-sm font-medium"
        >
          + Add Member
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">
          <p className="mb-2">No family members found.</p>
          ...
          <table className="w-full text-sm">
            <thead className="bg-amber-100 text-amber-900">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-semibold">Gen</th>
                <th className="text-left px-4 py-3 font-semibold">Gender</th>
                <th className="text-left px-4 py-3 font-semibold">Born</th>
                <th className="text-left px-4 py-3 font-semibold">Died</th>
                <th className="text-left px-4 py-3 font-semibold">Father</th>
                <th className="text-left px-4 py-3 font-semibold">Mother</th>
                <th className="text-left px-4 py-3 font-semibold">Spouse</th>
              </tr>
            </thead>
      ...
      <p className="text-xs text-gray-400 mt-4">
        {persons.length} member{persons.length !== 1 ? "s" : ""} total
      </p>
    </div>
  );
}
